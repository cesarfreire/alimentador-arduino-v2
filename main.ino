...
void lerbotoes()
{
  ...
  if (valorbotao > 340 && valorbotao < 350)
    t_butUp = 1;                    //Botão CIMA pressionado? Seta flag
  ...
  else if (valorbotao > 140 && valorbotao < 150)
    t_butDown = 1;                    //Botão BAIXO pressionado? Seta flag
  ...
  else if (valorbotao > 500 && valorbotao < 520)
    t_butP = 0x01;                 //Botão P pressionado? Seta flag
  ...
  {                                //Se for sim...
    ...
    t_butM = 0x01;                 //Botão M pressionado? Seta flag
  if (valorbotao > 1020 && t_butM) //Botão M solto e flag setada?                         // BOTÃO LADO ESQUERDO
  {                                //Se for sim...
    t_butM = 0;                    //Limpa flag
    f_butM = 1;                    //Ativa função Botao M
  }
  else if (valorbotao > 740 && valorbotao < 750)
    t_butS = 0x01;                 //Botão M pressionado? Seta flag
...
void changeMenu() //Função para modificar o menu atual
{
  ...
  {               //Se a função do botão BAIXO foi ATIVA                                                                 //Sim...
    ...
    if (menu < 0x01)
      menu = 0x06; //Se menu menor que 1, volta a ser 6
...
void horario1() //Função para definir o intervalo
{
  ...
  if (horasval1 > 23)
    horasval1 = 1; //Se for maior que 23 horas, retorna ao valor 1
  if (horasval1 < 1)
    horasval1 = 23; //Se for menor que 1 hora, retorna ao valor de 23 horas
  if (minutosval1 > 60)
    minutosval1 = 0; //Se for maior que 60 minutos, retorna a 0
  if (minutosval1 < 0)
    minutosval1 = 59; //Se for menor que 0 minutos, retorna a 60
  if (minutosval1 == 60)
    minutosval1 = 59;
...
void horario2() //Função para definir o intervalo
{
  ...
  if (horasval2 > 23)
    horasval2 = 1; //Se for maior que 23 horas, retorna ao valor 1
  if (horasval2 < 1)
    horasval2 = 23; //Se for menor que 1 hora, retorna ao valor de 23 horas
  if (minutosval2 > 60)
    minutosval2 = 0; //Se for maior que 60 minutos, retorna a 0
  if (minutosval2 < 0)
    minutosval2 = 59; //Se for menor que 0 minutos, retorna a 59
  if (minutosval2 == 60)
    minutosval2 = 59; //Se for igual a 60, então fica 59
...
void horario3() //Função para definir o intervalo
{
  ...
  if (horasval3 > 23)
    horasval3 = 1; //Se for maior que 23 horas, retorna ao valor 1
  if (horasval3 < 1)
    horasval3 = 23; //Se for menor que 1 hora, retorna ao valor de 23 horas
  if (minutosval3 < 0)
    minutosval3 = 59; //Se for menor que 0 minutos, retorna a 59
  if (minutosval3 == 60)
    minutosval3 = 59; //Se for igual a 60, então fica 59
...
void definirquant() //Menu para definir a quantidade de ração
{
  ...
  if (numvoltas > 5)
    numvoltas = 1; //Se o número de voltas for maior que 5, retorna ao valor 1
  if (numvoltas < 1)
    numvoltas = 5; //Se o número de voltas for menor que 1, retorna o valor 5
...
void horaprogramada()
{
  if ((horasval1 == dataehora.hour) && (minutosval1 == dataehora.minute))
  { //Se os valores de horas e minutos escolhidos forem iguais ao horário atual

    if (rodar == true)
    {                //Se a variável rodar for TRUE
      rotacionar();  //Chama a função de rotacionar o motor
      rodar = false; //Seta a variável para FALSE
    }
  }

  if ((horasval2 == dataehora.hour) && (minutosval2 == dataehora.minute))
  { //Se os valores de horas e minutos escolhidos forem iguais ao horário atual
  ...
  if ((horasval3 == dataehora.hour) && (minutosval3 == dataehora.minute))
  { //Se os valores de horas e minutos escolhidos forem iguais ao horário atual
...
void rotacionar()
{ //Função de rotacionar o motor
  for (int i = 0; i < numvoltas; i++)
  {                    //Laço FOR para girar o motor de acordo com a escolha do usuário na variável
